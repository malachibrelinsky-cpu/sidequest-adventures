...
function QuestChatPage() {
  ...
  const onPickFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { error } = await supabase.from("quest_messages").insert({
        post_id: questId, user_id: user.id, body: url,
      });
        ...
        <div className="bento-card flex-1 flex flex-col overflow-hidden min-h-[60vh]">
                ...
                <div key={m.id} className={`flex gap-2 ${mine ? "justify-end" : "justify-start"}`}>
                  ...
                  {media ? (
                    <div className={`max-w-[75%] overflow-hidden rounded-2xl ${mine ? "rounded-br-sm" : "rounded-bl-sm"} bg-muted/30`}>
                      {!mine && <p className="text-[10px] font-semibold opacity-70 px-3 pt-2">{author?.display_name ?? "Quester"}</p>}
                      {media.kind === "image" ? (
          ...
          <form onSubmit={send} className="border-t border-border p-3 flex gap-2 items-center">
            ...
            <input
              ...
              maxLength={2000}
              placeholder={uploading ? "Uploading…" : "Message the crew…"}
